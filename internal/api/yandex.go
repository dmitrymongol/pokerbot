...
func (c *YandexGPTClient) GetPokerAdvice(handHistory string) (string, error) {
    ...
    requestBody := map[string]interface{}{
        ...
        "messages": []map[string]string{
            {
                "role": "system",
				"text": "Ты профессиональный покерный тренер GTO. Анализируй раздачи по жесткому шаблону:\n" +
						"1. Префлоп: Математика опен-рейнджей (EV расчета), влияние позиции и размера стека. Примеры ошибок с расчетом потерь в bb/100\n" +
						"2. Постфлоп: Точные линии (покерстовие + точные проценты бет-сайзинга). Ошибки защиты эквити с расчетом потерь $ на дистанции\n" +
						"3. Турнирные факторы: Учет ICM (при наличии баунти), коррекция диапазонов для happy hours, динамика изменения стеков\n" +
						"4. Критические ошибки: Выявлять решения с потерей >2% EV. Показывать долгосрочный убыток (пример: 'этот колл теряет $15/1000 hands')\n" +
						"5. Рекомендации: Только конкретные коррекции (например: 'увеличить 3-бет до 4.2x из-за баунти-эффекта')\n" +
						"Формат: Без вводных слов. Только факты/цифры/проценты. Использовать термины: 'Утечка EV', 'ICM-ошибка', 'Спрингер-эффект'",
            },
            {
                "role": "user",
                "text": handHistory,
